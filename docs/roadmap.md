...
- Add operation counters to every algorithm. Current coverage includes core
  ...
  and CH query stats, frontier-partition experiments, the Johnson
  negative-weight baseline, the Thorup-like integer baseline, and optional Rust
  single-source wrappers with deterministic wrapper-level structural counters.
